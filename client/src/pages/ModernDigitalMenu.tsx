    ...
    <div className={`min-h-screen bg-white menu-font pb-24 ${storeData?.darkMode ? 'dark-menu' : ''}`}>
      ...
      <div className="max-w-6xl mx-auto px-4 pb-24">
        {searchQuery ? (
          ...
          <>
            {/* Carrossel de Produtos em Destaque - Formato horizontal compacto */}
            {featuredProducts.length > 0 && (
              <div className="py-4">
                <h2 className="text-lg font-bold mb-3 px-1 text-gray-900">Destaques</h2>
                <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 px-1">
                  {featuredProducts.map((product) => (
                    <div 
                      key={product.id} 
                      className="flex-shrink-0 w-44 bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-200"
                    >
                          ...
                          <div className="flex items-center gap-2">
                            {product.isPromotion && (
                              <span className="text-xs text-gray-400 line-through">
                                R$ {(parseFloat(product.price) * 1.3).toFixed(2).replace('.', ',')}
                              </span>
                            )}
                ...
                {storeData.sections
                  ...
                  .map((section) => (
                    <button
                      ...
                      className={`flex-shrink-0 px-5 py-2.5 rounded-full text-sm font-semibold transition-all duration-300 shadow-sm hover:shadow-md ${
                        activeSection === section.id 
                          ? 'text-white transform scale-105' 
                          : 'bg-white border-2 hover:scale-102'
                      }`}
                      style={{
                        backgroundColor: activeSection === section.id ? (storeData?.primaryColor || '#FF6B35') : 'white',
...
function ModernProductCard({ product, onAddToCart, storeData }: { product: any; onAddToCart: (product: any) => void; storeData?: StoreData }) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 px-4 py-2 sm:py-3 z-50">
        <div className="flex justify-around items-center max-w-lg mx-auto">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] sm:text-xs mt-1 font-medium text-gray-600">Início</span>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] sm:text-xs mt-1 font-medium text-gray-600">Buscar</span>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] sm:text-xs mt-1 font-medium text-gray-600">Favoritos</span>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-[10px] sm:text-xs mt-1 font-medium text-gray-600">Perfil</span>
          </div>
        </div>
      </div>
